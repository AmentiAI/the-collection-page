...
async function generateMutantMonsterImage(prompt: string): Promise<{ imageUrl: string; imageBlobUrl: string }> {
  ...
  try {
    ...
    const blobName = `admin-regenerate/regen-${timestamp}.png`
    ...
    console.error('[admin-regenerate][blob upload]', blobError)
...
export async function POST(
  ...
  try {
    ...
    // Generate new image using OpenAI (no allowance check for admin)
    const { imageUrl, imageBlobUrl } = await generateMutantMonsterImage(prompt)
