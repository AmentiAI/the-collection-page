...
async function generateMegaMonsterImage(prompt: string): Promise<{ imageUrl: string; imageBlobUrl: string }> {
  ...
  try {
    ...
    const blobName = `mega-monsters/mega-${timestamp}.png`
...
export async function POST(
  ...
  try {
    ...
    // Update the record with generated image data
    const updateResult = await client.query(
      `
      UPDATE mega_monsters
      SET image_data = $1, image_blob_url = $2, updated_at = NOW()
      WHERE id = $3
      RETURNING *
      `,
      [imageUrl, imageBlobUrl, id]
    )
    ...
    console.error('Mega monster generation error:', error)
    return NextResponse.json(
      {
        success: false,
