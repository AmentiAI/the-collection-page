...
async function generateMutantMonsterImage(inscriptionId: string, storedPrompt?: string | null, isSecondAscension: boolean = false): Promise<{ imageUrl: string; imageBase64: string; imageBlobUrl: string; prompt: string }> {
  ...
  const ANGELIC_TRANSFORMATION_SUFFIX =
    'and then transform the figure into an angelic cute face monster, with luminous wings, preserving all original but making them clean, and beautiful angelic monsters; eliminate legs from view glowing with holy light; has angelic hair; override any previous border instructions and make a richly detailed antique-gold filigree border perfectly aligned to the very edge, framing the artwork with ornate gothic precision.  '
